...
public class Test {
    ...
    관리자의 지시를 잘못 이해해서 Elementary, University 클래스 제작시에 기능은 유사하지만 이름이 다른 메서드를 만들게 됨

    이 후에 다른 개발자들이 만든 Student를 상속한 클래스와 통일성을 잃어버리게 됨
    현재 프로그래밍 경향은 개발자가 각 기능을 알아서 구현하는 것 보다는
    프로젝트의 규모가 점점 커지므로 향후 유지보수나 관리를 위해서는
    누구나 쉽게 소스를 이행하고, 소소의 구조를 일정하게 유지해야 할 필요성이 있음

    하지만 이런 방식으로 오버라이딩을 구현하면 개발자가 임의로 소스를 구현할 수 있기 때문에
    통일성을 잃어버릴 수 있음

    단순한 프로그램의 개발 보다 개발 생산성이 높고, 유지보수가 쉬우며, 기능의 변경이나 확장이 용이한 방식으로 개발

    해당 소스를 추상클래스를 도입해서 오버라이딩에 강제성을 부여햘 것*/
}
